...
export default function AttendanceTracker() {
  ...
  const shareAbsentList = () => {
    const absentMembers = getFilteredMembers().filter(member => 
      attendanceStatus[member.name] !== 'present'
    );
    
    if (absentMembers.length === 0) {
      alert('No absent members to share');
      return;
    }

    const message = `Absent Members - ${selectedSection}:\\n${absentMembers.map(m => `- ${m.name}`).join('\\n')}`;
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-4">
          <h2 className="text-xl font-bold text-red-600 mb-4 text-center">Absent</h2>
        ...
        {/* Present Members */}
        <div className="bg-white rounded-lg shadow-md p-4">
          <h2 className="text-xl font-bold text-green-600 mb-4 text-center">Present</h2>
          <div className="space-y-2">
            {getFilteredMembers()
              .filter(member => attendanceStatus[member.name] === 'present')
              .map((member) => (
